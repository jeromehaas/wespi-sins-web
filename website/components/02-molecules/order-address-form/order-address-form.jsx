	...
	return (
		<div className={ `${ className } order-address-form` }>
			...
			<div className="order-address-form__form form">
				...
				<InputField className="form__input-field" category="address" id="phone" label="Telefonnummer*" placeholder="Ihre Telefonnummer" error={ formState.errors['address'] } type="text" register={ register } validation={ { 	required: { value: true, message: 'Bitte geben Sie Ihre Telefonnummer an' }, 	pattern: { value: /^(0041|\+41)?\s?[0-9]\s?(\d{2})[\s.-]?(\d{2,4})[\s.-]?(\d{0,2})[\s.-]?(\d{0,2})$/i, message: 'Bitte geben Sie eine gültige Telefonnummer an' } } } />
				...
				<InputField className="form__input-field" category="address" id="street" label="Strasse und Hausnummer" placeholder="Ihre Strasse und Hausnummer" error={ formState.errors['address'] } type="text" register={ register } validation={ { 	required: { value: true, message: 'Bitte geben Sie Ihre Strasse und Hausnummer an' } } } />
