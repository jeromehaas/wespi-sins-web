...
// COMPONENT
const TextArea = ({ className = '', category, id = '', label = '', placeholder = '', register = null, validation = null, error = null, readOnly = false }) => {
	...
	return (
		<div className={ `${ className } text-area ${ error && error[id] ? 'text-area--error' : null }` }>
			...
			<textarea className="text-area__input" id={ id } placeholder={ placeholder } { ...register(`${ category }.${ id }`, validation) } rows={ 6 } readOnly={ readOnly } />
